            ...
            <div class="p-4 border-t flex items-center gap-3">
                ...
                <div class="flex-1">
                    <p class="text-sm font-medium text-gray-700">
                        {{ Auth::guard('admin')->user() ? 'Admin' : Auth::guard('user')->user()->full_name }}</p>
            ...
            <header class="bg-white border-b px-6 py-4 flex justify-between items-center">
                ...
                <div class="flex items-center gap-4">
                    <button class="relative text-gray-500 hover:text-primary">
                        ...
                        <span
                            class="absolute -top-1 -right-2 bg-red-500 text-white text-[10px] px-1 rounded-full">2</span>
                    </button>
                    @php
                        $route = null;
                        ...
                        <div class="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center">
                            <a href="{{ route($route) }}">
                                {{ Auth::guard('admin')->user()
                                    ? strtoupper(substr(Auth::guard('admin')->user()->full_name, 0, 2))
                                    : strtoupper(substr(Auth::guard('user')->user()->full_name, 0, 2)) }}</a>
                        </div>
                        <span
                            class="text-sm text-gray-600">{{ Auth::guard('admin')->user() ? Auth::guard('admin')->user()->full_name : Auth::guard('user')->user()->full_name }}</span>
                    </div>
                </div>
            </header>
